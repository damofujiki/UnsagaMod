...
public class ChestCapability {
	...
	public static class DefaultImpl implements IChestCapability{
		...
		@Override
		public List<TrapChest> getTraps() {
			// TODO 自動生成されたメソッド・スタブ
			return this.traps;
		}
		...
		@Override
		public void setTraps(List<TrapChest> traps) {
			// TODO 自動生成されたメソッド・スタブ
			this.traps = traps;
		}
	...
	public static class Storage extends CapabilityStorage<IChestCapability>{
		...
		public void readNBT(NBTTagCompound comp, Capability<IChestCapability> capability, IChestCapability instance, EnumFacing side) {
			...
			if(comp.hasKey("traps")){
				instance.setTraps(UtilNBT.readListFromNBT(comp,"traps",TrapChest.RESTORE));
			}else{
	...
	public static enum ChestTreasureType{
		...
		public @Nullable ItemStack createTreasure(Random rand,int level,ItemFactory factory){
			switch(this){
			case ITEM:
				...
				List<ItemStack> stacks = factory.createMerchandises(5, generateLevel, cate, Sets.newHashSet(UnsagaMaterials.instance().merchandiseMaterial));
				Collections.shuffle(stacks);
				ItemStack rt = stacks.get(0);
				return rt;
			case MONEY:
				int num = rand.nextInt(level/10+1);
				return new ItemStack(Items.GOLD_NUGGET,MathHelper.clamp_int(num, 1, 10));
			case TABLET:
				return TabletRegistry.getRandomTabletByWeight(rand).getStack(1);
			default:
				break;
	...
	public static CapabilityAdapterFrame<IChestCapability> adapterBase = UnsagaMod.capabilityFactory.create(capabilityAdapter);
	public static ComponentCapabilityAdapterEntity<IChestCapability> adapterEntity =adapterBase.createChildEntity("chest");

	public static ComponentCapabilityAdapterTileEntity<IChestCapability> adapterTE = adapterBase.createChildTileEntity("techest");
	...
	public static void register(){
		...
		adapterEntity.registerAttachEvent((inst,capa,facing,ev)->{
			if(!inst.hasInitialized()){
				...
				inst.setTraps(ChestHandler.getInitializedTraps(inst, rand));

...
public class MaterialListCategory implements IRecipeCategory<MaterialListWrapper>{
	...
	@Override
	public void setRecipe(IRecipeLayout recipeLayout, MaterialListWrapper recipeWrapper) {
		// TODO 自動生成されたメソッド・スタブ
	...
	public void setRecipe(IRecipeLayout recipeLayout, MaterialListWrapper recipeWrapper, IIngredients ingredients) {
		...
		if(recipeWrapper.isMinsagaForging()){
			MinsagaForging.Material m = ingredients.getInputs(MinsagaForging.Material.class).get(0).get(0);
			recipeLayout.getIngredientsGroup(MinsagaForging.Material.class).init(0, true, new MinsagaForgingMaterialRenderer(), 39, 16, 16, 16, 0,0);
			recipeLayout.getIngredientsGroup(MinsagaForging.Material.class).set(0,ingredients.getInputs(MinsagaForging.Material.class).get(0));
		}else{

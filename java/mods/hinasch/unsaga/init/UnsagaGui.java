...
import mods.hinasch.unsaga.core.inventory.container.ContainerSmithMinsaga;
	...
	public static enum Type implements IGuiAttribute{
		...
		public Container getContainer(World world,EntityPlayer player,XYZPos pos){
			...
			case TABLET:
				...
				break;
			case SMITH:
				if(InteractionInfoCapability.adapter.hasCapability(player) && InteractionInfoCapability.adapter.getCapability(player).getMerchant().isPresent()){
					return new ContainerSmithUnsaga((IMerchant) InteractionInfoCapability.adapter.getCapability(player).getMerchant().get(), world, player);
				}
				break;
			case BARTERING:
				if(InteractionInfoCapability.adapter.hasCapability(player) && InteractionInfoCapability.adapter.getCapability(player).getMerchant().isPresent()){
					return new ContainerBartering(world, player, (IMerchant) InteractionInfoCapability.adapter.getCapability(player).getMerchant().get());
				}
				break;
			case CHEST:
				if(InteractionInfoCapability.adapter.hasCapability(player) && InteractionInfoCapability.adapter.getCapability(player).getChest().isPresent()){
					Entity chest = InteractionInfoCapability.adapter.getCapability(player).getChest().get();
...
//				}
			...
			case BINDER:
				...
				break;
			case SMITH_MINSAGA:
				return new ContainerSmithMinsaga(player);
			case MANUAL_ABILITY:
		...
		public GuiContainer getGui(World world,EntityPlayer player,XYZPos pos){
			...
			case TABLET:
				...
				break;
			case SMITH:
				return new GuiSmithUnsaga(new NpcMerchant(player,ChatHandler.createChatComponent("smith")),world,player);
			case BARTERING:
...
//				}
			...
			case BINDER:
				...
				break;
			case SMITH_MINSAGA:
				return new GuiSmithMinsaga(world,player);
			case MANUAL_ABILITY:
//				ITextMenuAdapter adapter = new AbilityTextMenuAdapter();
		...
		public void onGuiOpen(PacketOpenGui pgo,MessageContext ctx,EntityPlayer ep ){
			if(Lists.newArrayList(Type.SMITH,Type.BARTERING).contains(fromMeta(pgo.getGuiNumber()))){
				Entity entity = ep.worldObj.getEntityByID(pgo.getComp().getInteger("villager"));

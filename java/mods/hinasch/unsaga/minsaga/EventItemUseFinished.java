...
public class EventItemUseFinished {
	...
	public void onFoodEaten(LivingEntityUseItemEvent.Finish ev){
		if(ev.getItem().getItem() instanceof ItemFood){
			List<MinsagaForging.Ability> list = MinsagaUtil.getForgedArmors(ev.getEntityLiving()).stream().flatMap(in -> ForgingCapability.adapter.getCapability(in).getAbilities().stream()).collect(Collectors.toList());

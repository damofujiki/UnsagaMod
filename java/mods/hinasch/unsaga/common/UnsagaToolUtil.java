...
public class UnsagaToolUtil {
    ...
    public static UnsagaMaterial getMaterial(ItemStack is){
    	if(UnsagaMaterialTool.adapter.hasCapability(is)){
    		return UnsagaMaterialTool.adapter.getCapability(is).getMaterial();
    	}
    	return def;
    }
    ...
    public static void setMaterial(ItemStack is,UnsagaMaterial m){
    	if(UnsagaMaterialTool.adapter.hasCapability(is)){
    		UnsagaMaterialTool.adapter.getCapability(is).setMaterial(m);
    	}
    }

    public static float getEfficiencyOnProperMaterial(ItemStack is){
    	if(UnsagaMaterialTool.adapter.hasCapability(is)){
    		return UnsagaMaterialTool.adapter.getCapability(is).getMaterial().getToolMaterial().getEfficiencyOnProperMaterial();
    	}
    ...
    public static int getItemEnchantability(ItemStack is){
    	if(UnsagaMaterialTool.adapter.hasCapability(is)){
    		return UnsagaMaterialTool.adapter.getCapability(is).getMaterial().getToolMaterial().getEnchantability();
    	}

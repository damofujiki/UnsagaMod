...
//					this.callThunderAt(context.getWorld(), pos);
	...
	private void attackTarget(T context,EntityLivingBase target){
		target.attackEntityFrom(DamageSourceUnsaga.create(context.getPerformer(), context.getModifiedStrength().lp(), General.MAGIC)
				.setSubTypes(Sub.ELECTRIC),context.getModifiedStrength().hp());
	}

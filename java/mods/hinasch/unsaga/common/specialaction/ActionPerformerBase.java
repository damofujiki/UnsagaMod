...
public abstract class ActionPerformerBase<T, V> implements IActionPerformer<T,V>{
	...
	protected IActionPerformer.TargetType targetType;
...
//		UnsagaMod.logger.trace(this.getClass().getName(), type);
	...
	public ActionPerformerBase setArticle(ItemStack is){
	...
	public boolean checkItemCost(){
		if(this.getArticle().isPresent()){
			int durability = this.getArticle().get().getMaxDamage() - this.getArticle().get().getItemDamage();
	...
	public void broadCastMessage(String msg){
		...
		if(this.getPerformer() instanceof EntityPlayer){
			...
			this.getWorld().getEntitiesWithinAABB(EntityLivingBase.class, this.getPerformer().getEntityBoundingBox().expandXyz(10.0D))
			.forEach(in ->{

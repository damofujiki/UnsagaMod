...
public class DamageTypeAssociation {
	...
	private DamageTypeAssociation(){
		...
		map3.put(d -> d == DamageSource.onFire, new Attribute(0,General.MAGIC,Sub.FIRE));
		map3.put(d -> d == DamageSource.inFire, new Attribute(0,General.MAGIC,Sub.FIRE));
		map3.put(d -> d == DamageSource.lava, new Attribute(0,General.MAGIC,Sub.FIRE));
		map3.put(d -> d == DamageSource.dragonBreath, new Attribute(0,General.MAGIC,Sub.FIRE));
		map3.put(d -> d == DamageSource.lightningBolt, new Attribute(0,General.MAGIC,Sub.ELECTRIC));
		...
		map3.put(d -> d.getSourceOfDamage() instanceof EntityArrow, new Attribute(3,General.SPEAR));
		map3.put(d -> d.isExplosion(), new Attribute(0,General.MAGIC,Sub.FIRE));
		map3.put(d -> d.isFireDamage(), new Attribute(0,Sub.FIRE));
		map3.put(d -> {
			if(d.getSourceOfDamage() instanceof EntityArrow && EntityStateCapability.adapter.hasCapability(d.getSourceOfDamage())){
				StateArrow state = (StateArrow) EntityStateCapability.adapter.getCapability(d.getSourceOfDamage()).getState(StateRegistry.instance().stateArrow);

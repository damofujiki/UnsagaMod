...
import mods.hinasch.unsaga.core.inventory.container.MatrixAdapter;
...
public class SkillPanelBonus {
	...
	public static enum Type{LUCK,ALL,MAGIC,LPSTR,HEALTH,STR,ELEMENTS;
		...
		public Pair<IAttribute,AttributeModifier> getModifierPair(float f){
			...
			case LPSTR:
				return Pair.of(AdditionalStatus.DEXTALITY, new AttributeModifier(PANEL_UUID,NAME,f,op));
			case LUCK:
				return Pair.of(SharedMonsterAttributes.LUCK, new AttributeModifier(PANEL_UUID,NAME,f,op));
			case MAGIC:
				return Pair.of(AdditionalStatus.INTELLIGENCE, new AttributeModifier(PANEL_UUID,NAME,f,op));
			case STR:
	...
	public static void applyBonus(MatrixAdapter<ItemStack> matrix,EntityLivingBase living,boolean isLineBonus){
	...
	public static void refleshModifier(EntityLivingBase living,IAttribute attribute,AttributeModifier modifier){
		...
		UnsagaMod.logger.trace(attribute.getAttributeUnlocalizedName(), living.getEntityAttribute(attribute).getAttributeValue());
	}
	public static float calcAppliedBonus(MatrixAdapter<ItemStack> matrix,Type type,boolean isLineBonus){
		float multiply = isLineBonus ? 2.0F : 1.0F;
		return (calcBonus(matrix,type.getIconType()) * multiply) + calcBonus(matrix,Type.ALL);
	}
	public static float calcBonus(MatrixAdapter<ItemStack> matrix,Type type){
		return calcBonus(matrix,type.getIconType());
	}

	public static float calcBonus(MatrixAdapter<ItemStack> matrix,SkillPanel.IconType type){

...
public class UnsagaMaterialTool {


	@CapabilityInject(IUnsagaForgeableTool.class)
	public static Capability<IUnsagaForgeableTool> CAPA;
	public static final String SYNC_ID = "unsagaForgeableTool";

	public static ICapabilityAdapterPlan<IUnsagaForgeableTool> ica = new ICapabilityAdapterPlan(){
	...
	public static CapabilityAdapterFrame<IUnsagaForgeableTool> base = UnsagaMod.capabilityFactory.create(ica);
	public static ComponentCapabilityAdapterItem<IUnsagaForgeableTool> adapter = base.createChildItem("unsagaForgeableTool");

	static{
		adapter.setPredicate(ev -> {
//			UnsagaMod.logger.trace("capa", ev.getItemStack().getItem());
			return ev.getObject() instanceof IUnsagaMaterialSelector;
		});
	...
	public static class DefaultImpl implements IUnsagaForgeableTool{
		...
		int weight = 0;
		UnsagaMaterial material = UnsagaMaterials.instance().dummy;
		@Override
	...
	public static class Storage extends CapabilityStorage<IUnsagaForgeableTool>{
		...
		public void readNBT(NBTTagCompound comp, Capability<IUnsagaForgeableTool> capability,
			...
			if(comp.hasKey("material")){
				...
				UnsagaMaterial m = UnsagaMaterials.instance().getObject(new ResourceLocation(key));
				if(m!=null){
					instance.setMaterial(m);
				}else{
					instance.setMaterial(UnsagaMaterials.instance().dummy);
				}
...
//			UnsagaMod.logger.trace("capa", "attach");
			if(ev.getObject() instanceof IUnsagaMaterialSelector){
				if(!inst.hasInitialized()){
					inst.setMaterial(UnsagaMaterials.instance().dummy);
					inst.setWeight(1);

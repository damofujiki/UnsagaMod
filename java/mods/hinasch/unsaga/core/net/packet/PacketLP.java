				...
				if(player!=null){
					...
					if(entity!=null && LifePoint.adapter.hasCapability((EntityLivingBase) entity)){
						if(message.isRenderRequest()){
							...
							LifePoint.adapter.getCapability(living).markDirtyAndSetTick(true);
						}else{
...
//					return null;
					...
					if(entity instanceof EntityLivingBase && LPLogicManager.hasCapability((EntityLivingBase) entity)){

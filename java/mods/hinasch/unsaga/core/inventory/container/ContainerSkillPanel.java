...
public class ContainerSkillPanel extends ContainerBase{
	...
	protected InventoryBase invTemp;
	...
	public MatrixAdapter<ItemStack> getPanelMatrix() {
		return matrixPanel;
	}
	...
	public ContainerSkillPanel(EntityPlayer ep) {
		...
		this.invTemp = new InventoryBase(6);
		...
		ItemStackList panelList = data.getPanels(ep.getUniqueID());
	...
	public void undo(){
		...
		if(this.getEmptySlot().isPresent()){
			this.invTemp.setInventorySlotContents(this.getEmptySlot().getAsInt(), undoPanel!=null? undoPanel.copy() : null);
		}

...
public class ContainerSmithMinsaga extends ContainerBase{
	...
	public ContainerSmithMinsaga(EntityPlayer ep) {
...
//		UnsagaMod.logger.trace("id", this.buttonID);
		if(this.buttonID==GuiSmithMinsaga.ID_FORGE){

			List<XYZPos> list = WorldHelper.findNear(ep.worldObj, XYZPos.createFrom(ep), 5, 3, (w,p,scanner)->scanner.getBlock()==Blocks.ANVIL);
			if(list.isEmpty()){
				return;
			}
			...
			if(this.hasBaseTool() && this.hasMaterialStack() && this.hasExp(ep)){
				...
				if(ForgingCapability.adapter.hasCapability(newTool) && MinsagaForging.instance().getMaterialFromItemStack(materialStack).isPresent()){
					MinsagaForging.Material material = MinsagaForging.instance().getMaterialFromItemStack(materialStack).get();
					ForgingCapability.adapter.getCapability(newTool).addNewForge(material);
					this.inventorySmith.setForged(newTool);
					this.inventorySmith.consumerItems();
					this.consumeExp(ep);
					this.updateWeaponModifier(newTool);
					this.updateAnvilState(ep.worldObj, ep, pos, anvil);
				}
			}
		}
	}
	...
	public void updateWeaponModifier(ItemStack forged){
		...
		if(ForgingCapability.adapter.hasCapability(forged)){
			IMinsagaForge capa = ForgingCapability.adapter.getCapability(forged);

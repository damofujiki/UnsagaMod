...
import mods.hinasch.unsaga.util.UnsagaVillager;
import mods.hinasch.unsaga.util.UnsagaVillager.IUnsagaVillager;
...
//	protected int isPriceUp;
	...
	public ContainerBartering(World world,EntityPlayer ep,IMerchant merchant){
		...
		this.dummyInv = new InventoryBase(10); //インベントリの連携はいらないがないとダメ…？
		...
		if(this.theMerchant instanceof EntityVillager){
			...
			if(WorldHelper.isServer(world)){
				...
				if(UnsagaVillagerCapability.adapter.hasCapability(villager)){
					ItemStackList merchandises = UnsagaVillagerCapability.adapter.getCapability(villager).getMerchandises();
					ItemStackList secrets = UnsagaVillagerCapability.adapter.getCapability(villager).getSecretMerchandises();
		...
		return InventoryHandler.create(this.invSell).toStream(0, this.invSell.getSizeInventory())
				.mapToInt(in -> {
					if(in.getStack()!=null && MerchandiseCapability.adapter.hasCapability(in.getStack())){
						return MerchandiseCapability.adapter.getCapability(in.getStack()).getPrice(in.getStack()) * in.getStack().stackSize;
					}
					return 0;
				}).sum();
	}
	...
	public void onContainerClosed(EntityPlayer par1EntityPlayer)
	{
		...
		if(this.theMerchant instanceof EntityVillager && UnsagaVillagerCapability.adapter.hasCapability((Entity) this.theMerchant)){
			EntityVillager villager = (EntityVillager) this.theMerchant;
			UnsagaVillagerCapability.adapter.getCapability(villager).setMerchandises(ItemStackList.of(this.invMerchant,0,9));
			UnsagaVillagerCapability.adapter.getCapability(villager).setSecretMerchandises(ItemStackList.of(this.invMerchant,9,9));

		}
		if(InteractionInfoCapability.adapter.hasCapability(ep)){
			InteractionInfoCapability.adapter.getCapability(ep).setMerchant(Optional.empty());
		}
	...
	public void onPacketData() {
		...
		case GuiBartering.BUTTON_GET_SECRET:
			if(!this.behavior.hasDisplayedSecrets()){
				int level = SkillPanelAPI.getHighestPanelLevel(ep, SkillPanelRegistry.instance().artiste).getAsInt();
				ItemStackList secrets = this.behavior.createSecretMerchandises(level);
				for(int i=0;i<secrets.size();i++){
					Slot slot = this.getSlotFromInventory(invMerchant, 9+i);
					slot.putStack(secrets.getAllowNull(i));
...
//		UnsagaMod.logger.trace("slot", par1);
		if(par1>=MERCHANDISE_SLOT_START && MERCHANDISE_SLOT_START+18>par1 ){
			...
			if(player instanceof EntityPlayerMP){
				...
				HSLib.core().getPacketDispatcher().sendTo(this.getSyncPacketToClient(player), (EntityPlayerMP) player);
			}

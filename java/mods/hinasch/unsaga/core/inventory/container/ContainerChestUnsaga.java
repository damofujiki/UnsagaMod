...
//		this.abilities = UnsagaMod.abilities;
	...
	public void activateTraps(){
		this.ep.closeScreen();
		boolean flag = false;
		Queue<TrapChest> queue = new ArrayBlockingQueue(10);
		this.chest.getCapability().getTraps().forEach(in -> queue.offer(in));
		for(int i=0;i<queue.size();i++){
			TrapChest trap = queue.poll();
			trap.activate(chest, openPlayer);
			if(trap==TrapChestRegistry.NEEDLE){
...
//	}
	...
	public void divine(EnumActionResult result,Random rand){
		...
		level = MathHelper.clamp_int(level, 1, 100);
	...
	public void onPacketData() {
		...
		case GuiChest.OPEN:
			...
			break;
		case GuiChest.UNLOCK:
			if(SkillPanelAPI.hasPanel(ep, SkillPanelRegistry.instance().locksmith)){
				SkillPanel skill = SkillPanelRegistry.instance().locksmith;
				EnumActionResult result = ChestHandler.tryInteraction(skill, rand, chest.getCapability(), SkillPanelAPI.getHighestPanelLevel(openPlayer, skill).getAsInt());
				this.unlock(result);
			}
			break;
		case GuiChest.DIVINATION:
			if(SkillPanelAPI.hasPanel(ep, SkillPanelRegistry.instance().fortuneTeller)){
				SkillPanel skill = SkillPanelRegistry.instance().fortuneTeller;
				EnumActionResult result = ChestHandler.tryInteraction(skill, rand, chest.getCapability(), SkillPanelAPI.getHighestPanelLevel(openPlayer, skill).getAsInt());
				this.divine(result,rand);
			}
			break;
		case GuiChest.PENETRATION:
			if(SkillPanelAPI.hasPanel(ep, SkillPanelRegistry.instance().sharpEye)){
				SkillPanel skill = SkillPanelRegistry.instance().sharpEye;
				EnumActionResult result = ChestHandler.tryInteraction(skill, rand, chest.getCapability(), SkillPanelAPI.getHighestPanelLevel(openPlayer, skill).getAsInt());
				this.penetrate(result);
			}
			break;
		case GuiChest.DEFUSE:
			if(SkillPanelAPI.hasPanel(ep, SkillPanelRegistry.instance().defuse)){
				SkillPanel skill = SkillPanelRegistry.instance().defuse;
				EnumActionResult result = ChestHandler.tryInteraction(skill, rand, chest.getCapability(), SkillPanelAPI.getHighestPanelLevel(openPlayer, skill).getAsInt());
				this.defuse(result);
			}
			break;
	...
	public void syncMessage(){
		if(!this.message.isEmpty()){
			if(WorldHelper.isServer(this.openPlayer.getEntityWorld())){
				HSLib.core().getPacketDispatcher().sendTo(this.getSyncPacketToClient(openPlayer), (EntityPlayerMP) this.openPlayer);
			}
		}

...
//			par3EntityPlayer.playSound(SoundEvents.entity_generic_explode, 1.0F, 1.0F / (itemRand.nextFloat() * 0.4F + 1.2F) + f * 0.5F);
		...
		if(!world.isRemote){
			if (entityplayer == null || isCreative) {
				...
				if(!linfinity && entityplayer instanceof EntityPlayer){
					...
					((EntityPlayer)entityplayer).inventory.clearMatchingItems(UnsagaItemRegistry.instance().ammo,-1,1,null);
				}
	...
	protected boolean canReload(ItemStack is,EntityPlayer ep){
		...
		if(ep.inventory.hasItemStack(new ItemStack(UnsagaItemRegistry.instance().ammo))) return true;
		return false;
	}
	...
	public static Predicate<AttachCapabilitiesEvent.Item> predicate = new Predicate<AttachCapabilitiesEvent.Item>(){
		...
		@Override
		public boolean apply(AttachCapabilitiesEvent.Item input) {
			// TODO 自動生成されたメソッド・スタブ
			return input.getObject() instanceof ItemGunUnsaga;
		}
	};
	public static ICapabilityAdapterPlan<IMusket> capabilityAdapter = new ICapabilityAdapterPlan<IMusket>(){
	...
	public static CapabilityAdapterFrame<IMusket> adapterBase = UnsagaMod.capabilityFactory.create(capabilityAdapter);
	public static ComponentCapabilityAdapterItem<IMusket> adapter = (ComponentCapabilityAdapterItem<IMusket>) adapterBase.createChildItem("musket")

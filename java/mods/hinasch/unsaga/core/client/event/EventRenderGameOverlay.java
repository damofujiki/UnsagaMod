...
//			}
			...
			if(SkillPanelAPI.hasPanel(ClientHelper.getPlayer(), SkillPanelRegistry.instance().watchingOut)){
				if(e.getDistanceToEntity(ClientHelper.getPlayer())<10.0D){
					return true;
				}
...
//			//敵がdetectデバフがあるか、もしくはwatchingOutスキルがあるか（範囲制限あり）
			...
			if(LifePoint.adapter.hasCapability(living) && UnsagaMod.configHandler.isEnabledLifePointSystem()){

				ILifePoint lpCapability = LifePoint.adapter.getCapability(living);
				boolean isNearPlayer = ClientHelper.getPlayer().getDistanceToEntity(e.getEntity())<8.0D;
				if(HSLibs.isSameTeam(ClientHelper.getPlayer(), living) ||(UnsagaMod.configHandler.isEnableRenderNearMonsterLP() && isNearPlayer)){
		...
		public void onRenderLivingPost(RenderLivingEvent.Post e){
			this.renderLivingLPDamage(e,new XYZPos(e.getX(),e.getY(),e.getZ()));
			this.renderTarget(e,new XYZPos(e.getX(),e.getY(),e.getZ()));
...
//
			EntityPlayer clientPlayer = ClientHelper.getPlayer();
			XYZPos offset = UnsagaMod.configHandler.getRenderLPOffset();
			if(UnsagaMod.configHandler.isEnabledLifePointSystem() && LifePoint.adapter.hasCapability(clientPlayer)){

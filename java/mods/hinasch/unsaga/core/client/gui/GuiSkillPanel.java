...
//		this.addButton(BUTTON_UNDO,  i + (18*1)+2, j + 30 +(18*0), 30, 20 ,"undo");
	...
	protected void drawGuiContainerForegroundLayer(int par1,int par2)
	{
		...
		MatrixAdapter<ItemStack> matrix = containerSkill.getPanelMatrix();
...
//		UnsagaXPCapability.displayAdditionalXP(ep, fontRendererObj, EnumSet.of(Type.SKILL));
		...
		fontRendererObj.drawString("Panel Bonus", -60,0,0xffffff);
		fontRendererObj.drawString(String.format("STR:%.2f", str), -60,16,0xffffff);
		fontRendererObj.drawString(String.format("HP:%.2f", health), -60,16*2,0xffffff);
		fontRendererObj.drawString(String.format("LUCK:%.2f", luck), -60,16*3,0xffffff);
		fontRendererObj.drawString(String.format("DEX:%.2f", lpstr), -60,16*4,0xffffff);
		fontRendererObj.drawString(String.format("INT:%.2f", magic), -60,16*5,0xffffff);
		fontRendererObj.drawString(String.format("ELM:%.2f", elements), -60,16*6,0xffffff);
	}

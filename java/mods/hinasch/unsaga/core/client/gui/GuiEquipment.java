...
public class GuiEquipment extends GuiContainerBase{
	...
	public void initGui()
	{
		...
		this.addIcon(new IconMapSkill(ROAD_ADVISER,8,65,0,200,true,reg.roadGuide));
		this.addIcon(new IconMapSkill(CAVERN_EXPLORER,8+16*1,65,16*1,200,true,reg.caveGuide));
		this.addIcon(new IconMapSkill(SPELL_BLEND,8+16*2,65,16*2,200,true,reg.magicBlend));
		this.addIcon(new IconMapSkill(EASY_REPAIR,8+16*3,65,16*3,200,true,reg.quickFix));
		this.addIcon(new IconMapSkill(WEAPON_FORGE,8+16*4,65,16*4,200,true,reg.toolCustomize));
		this.addIcon(new IconMapSkill(EAVESDROP,8+16*5,65,16*5,200,true,reg.eavesdrop));
		this.addIcon(DECIPHERING_POINT, 130, 38, 0, 184, true);
		this.addIcon(SKILL_POINT, 130, 58, 16, 184, true);
	}
	...
	public List<String> getIconHoverText(GuiContainerBase.Icon icon){
		...
		if(icon instanceof IconCondition){
			...
			list.add(HSLibs.translateKey(status.getType().getName()));

			if(HSLib.configHandler.isDebug()){
				list.add("Temp:"+status.getTemp());
				list.add("Humid:"+status.getHumid());
				list.add("HealTimer:"+HealTimerCalculator.calcHealTimer(player));
			}
		}

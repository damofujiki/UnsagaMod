...
//	private PriceDownUpPair discounts = new PriceDownUpPair(0,0);
	...
	public GuiBartering(IMerchant merchant,World world,EntityPlayer ep) {
		...
		this.addIcon(new IconSkill(BUTTON_DOWN_PRICE, -32, 32, 0, 168, true,SkillPanelRegistry.instance().monger));
		this.addIcon(new IconSkill(BUTTON_GET_SECRET, -32, 48, 16, 168, true,SkillPanelRegistry.instance().artiste));
		this.addIcon(new IconSkill(BUTTON_UP_PRICE, -32, 64, 32, 168, true,SkillPanelRegistry.instance().maharaja));
	...
	public List<String> getIconHoverText(Icon icon){
		...
		if(icon.id==BUTTON_DOWN_PRICE){
			...
			if(SkillPanelAPI.hasPanel(ep, SkillPanelRegistry.instance().monger)){
		...
		if(icon.id==BUTTON_UP_PRICE){
			...
			if(SkillPanelAPI.hasPanel(ep, SkillPanelRegistry.instance().maharaja)){
				list.add(HSLibs.translateKey("gui.unsaga.bartering.available"));
			}else{
				list.add(UnsagaTextFormatting.PROPERTY_LOCKED+HSLibs.translateKey("gui.unsaga.bartering.unavailable"));
			}
		...
		if(icon.id==BUTTON_GET_SECRET){
			...
			if(SkillPanelAPI.hasPanel(ep, SkillPanelRegistry.instance().artiste)){
				list.add(HSLibs.translateKey("gui.unsaga.bartering.available"));
			}else{
				list.add(UnsagaTextFormatting.PROPERTY_LOCKED+HSLibs.translateKey("gui.unsaga.bartering.unavailable"));
			}
	...
	protected void drawGuiContainerForegroundLayer(int par1,int par2)
	{
		...
		fontRendererObj.drawString(HSLibs.translateKey("gui.unsaga.bartering.amount.sell")+this.container.getSellPrice(),8,170,0xffffff);
		fontRendererObj.drawString(HSLibs.translateKey("gui.unsaga.bartering.amount.buy")+this.container.getBuyPrice(),8,186,0xffffff);

...
import mods.hinasch.unsaga.chest.ChestHandler;
import mods.hinasch.unsaga.chest.ChestHandler.BaseDifficulty;
import mods.hinasch.unsaga.chest.IChestBehavior;
import mods.hinasch.unsaga.chest.IChestCapability;
import mods.hinasch.unsaga.chest.TrapChest;
...
public class GuiChest extends GuiContainerBase{
	...
	public void initGui()
	{
		...
		this.addIcon(new IconSkill(DEFUSE, 24*1+8, iconYpos, 16, 168, true,reg.defuse));
		this.addIcon(new IconSkill(PENETRATION, 24*2+8, iconYpos, 32, 168, true,reg.sharpEye));
		this.addIcon(new IconSkill(DIVINATION, 24*3+8, iconYpos, 48, 168, true,reg.fortuneTeller));
		this.addIcon(new IconSkill(UNLOCK, 24*4+8, iconYpos, 64, 168, true,reg.locksmith));
		...
		this.addIcon(new IconTrap(NEEDLE, 84, 40, 0, 184, true,TrapChestRegistry.NEEDLE));
		this.addIcon(new IconTrap(POISON, 84+16*1, 40, 16, 184, true,TrapChestRegistry.POISON));
		this.addIcon(new IconTrap(EXPLODE, 84+16*2, 40, 32, 184, true,TrapChestRegistry.EXPLODE));
		this.addIcon(new IconTrap(SLIME, 84+16*3, 40, 48, 184, true,TrapChestRegistry.SLIME));
		//鍵かかっているかどうかも追加する
	}
	...
	protected void drawGuiContainerForegroundLayer(int par1,int par2)
	{
		...
		this.fontRendererObj.drawString(HSLibs.translateKey("word.chest")+" LV:"+this.theChest.getLevel(), 84, 20, 0x404040);
		this.fontRendererObj.drawString(HSLibs.translateKey(this.message), 16, 140, 0x404040);
		...
		if(id instanceof IconSkill){
			if(((IconSkill) id).isDisabled(this)){
				...
				if(SkillPanelAPI.hasPanel(openPlayer, ((IconSkill) id).getAssociatedSkill())){
					list.add(ChestHandler.getInteractionSuccessProb(((IconSkill) id).getBaseDifficulty(), this.theChest,
	...
	public static class IconSkill extends IconSkillAssociated<GuiChest>{
		...
		public BaseDifficulty getBaseDifficulty(){
			return ChestHandler.getBaseDifficulty(this.getAssociatedSkill());
		}

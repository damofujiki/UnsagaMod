...
public class EntityFireWall extends Entity{
	...
	final Spell property = SpellRegistry.instance().firewall;
	float damage = 1.0F;
	int life;
    ...
    public void onEntityUpdate()
    {
		...
		if(WorldHelper.isServer(worldObj)){
			...
			RangedHelper<EntityFireWall> ranged = RangedHelper.create(worldObj, this.owner, this.getEntityBoundingBox());
			...
			this.worldObj.getEntitiesWithinAABB(Entity.class, this.getEntityBoundingBox(),in -> in instanceof EntityThrowable || in instanceof EntityArrow)
		...
		if(this.getEntityWorld().rand.nextInt(8)==0){
			...
			ParticleHelper.MovingType.FLOATING.spawnParticle(worldObj, pos, EnumParticleTypes.FLAME, rand, 10, 0.1D);
		}

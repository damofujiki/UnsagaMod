		...
		if (inGround) {
			...
			if(i != inTile) {
				...
				if (isBurning() && (ticksInGround == 1)) {
					...
					worldObj.setLightFor(EnumSkyBlock.BLOCK,bpos, 0xff);
					worldObj.checkLightFor(EnumSkyBlock.BLOCK, bpos.east());
					worldObj.checkLightFor(EnumSkyBlock.BLOCK,bpos.west());
					worldObj.checkLightFor(EnumSkyBlock.BLOCK,bpos.down());
					worldObj.checkLightFor(EnumSkyBlock.BLOCK,bpos.up());
					worldObj.checkLightFor(EnumSkyBlock.BLOCK,bpos.north());
					worldObj.checkLightFor(EnumSkyBlock.BLOCK,bpos.south());
				}
			}
			return;
		} else {
...
//			zTile = movingobjectposition.blockZ;
			IBlockState state = worldObj.getBlockState(hitpos);
			...
			if (inTile == Blocks.GLASS_PANE || inTile == Blocks.FLOWER_POT || inTile == Blocks.GLOWSTONE) {
				...
				if (inTile instanceof BlockTNT) {
					...
					((BlockTNT)inTile).explode(worldObj, hitpos, state, this.shootingEntity);
					worldObj.setBlockToAir(hitpos);
					setDead();
				} else {
				}
				motionX = movingobjectposition.hitVec.xCoord - posX;
		...
		if (inGround) {
			for (int i = 0; i < 8; i++) {
				worldObj.spawnParticle(EnumParticleTypes.SNOWBALL,

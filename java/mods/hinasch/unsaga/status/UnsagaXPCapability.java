...
public class UnsagaXPCapability {
	...
	public static ICapabilityAdapterPlan<IUnsagaXP> ica = new ICapabilityAdapterPlan(){
	...
	public static CapabilityAdapterFrame<IUnsagaXP> base = UnsagaMod.capabilityFactory.create(ica);
	public static ComponentCapabilityAdapterEntity<IUnsagaXP> adapter = base.createChildEntity("skillPoint");
	static{
	...
	public static class DefaultImpl implements IUnsagaXP{
		...
		static final int DECIPHERING_MAX = 100;
		@Override
		public int getSkillPointPiece() {
			// TODO 自動生成されたメソッド・スタブ
		...
		public void addSkillPointPiece(int par1) {
			...
			this.skillPointPiece = MathHelper.clamp_int(skillPointPiece, 0, SKILLPOINT_MAX);
		...
		public void addDecipheringPoint(int par1) {
			// TODO 自動生成されたメソッド・スタブ
			this.decipheringPoint += par1;
			this.decipheringPoint = MathHelper.clamp_int(decipheringPoint, 0, DECIPHERING_MAX);
		}
	...
	public static void syncAdditionalXP(EntityPlayer ep){
		if(WorldHelper.isServer(ep.worldObj)){
			if(UnsagaXPCapability.hasCapability(ep)){
				...
				HSLib.core().getPacketDispatcher().sendTo(PacketSyncCapability.create(CAPA,capa), (EntityPlayerMP) ep);
			}
	...
	public void onTabletPickUp(EntityItemPickupEvent ev){
		if(ev.getItem().getEntityItem()!=null){
			if(ev.getItem().getEntityItem().getItem()==UnsagaMagicItems.instance().tablet){
	...
	public void onExpPickUp(PlayerPickupXpEvent ev){
		if(UnsagaXPCapability.hasCapability(ev.getEntityPlayer())){
			...
			if(ev.getEntityPlayer().hasAchievement(UnsagaMod.core.achievements.getTablet)){
